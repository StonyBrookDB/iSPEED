	...
	while(cin && getline(cin, input_line) && !cin.eof()) {
		...
		if (tile_id >= 0 && prev_id >= 0) {
			...
			if (prev_id == -1) {
				...
				low[2] = min(low[2], atof(fields[offset + 2].c_str()));
				high[0] = max(high[0], atof(fields[offset + 3].c_str()));
				high[1] = max(high[1], atof(fields[offset + 4].c_str()));
				high[2] = max(high[2], atof(fields[offset + 5].c_str()));
				count += atol(fields[offset + 6].c_str());
			} else {
				low[0] = atof(fields[offset].c_str());
				low[1] = atof(fields[offset + 1].c_str());
				low[2] = atof(fields[offset + 2].c_str());
				high[0] = atof(fields[offset + 3].c_str());
				high[1] = atof(fields[offset + 4].c_str());
				high[2] = atof(fields[offset + 5].c_str());
				count = atol(fields[offset + 6].c_str());
			}	
			fields.clear();
		}
