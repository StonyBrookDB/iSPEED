...
#define outbyte(cod,x) *(cod->p_data + (*cod->p_dataOffset)++) = x
...
{   
    ...
    //printf("Buffer val at beginning: %u and RNGC.low = %u, shift bits: %u\n", RNGC.buffer, RNGC.low, SHIFT_BITS);
    while(RNGC.range <= Bottom_value)     /* do we need renormalisation?  */
    {   
       /*
        printf("Normalizing 1 \n");
        for (size_t myidx = 0; myidx < 250; ++myidx) {
              printf("%x ", (*rc).p_data[myidx]);
 ...
 //        printf("Buffer val assigned to: %u and RNGC.low = %u, shift bits: %u\n", RNGC.buffer, RNGC.low, SHIFT_BITS);
        
         /*
        printf("Normalizing 1d \n");
        for (size_t myidx = 0; myidx < 250; ++myidx) {
              printf("%x ", (*rc).p_data[myidx]);
 	...
 	/*
        ...
        for (size_t myidx = 0; myidx < 1000; ++myidx) {
         ...
         printf("\n\n");
        */
        //printf("RNGC current %u\n", RNGC.low);
        RNGC.range <<= 8;
        RNGC.low = (RNGC.low<<8) & (Top_value-1);
       // printf("RNGC.low %u, top: %u\n", RNGC.low, Top_value);
        RNGC.bytecount++;
    }
}
#endif
...
{	
        code_value r, tmp;
       // printf("Before RNGC.range = %u, lt_f = %u, shift = %u\n", RNGC.range, lt_f, shift);
       // if (lt_f == 1909 || lt_f == 1910) {
                //exit(1);
       // }
	enc_normalize( rc );
        
       // printf("After normalize RNGC.range = %u, lt_f = %u, shift = %u\n", RNGC.range, lt_f, shift);
	r = RNGC.range >> shift;
	tmp = r * lt_f;
        // printf("After multiplying RNGC.low = %u, tmp = %u\n", RNGC.low, tmp);
